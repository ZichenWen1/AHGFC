...
def load_new_data(dataset, root):
    if dataset == 'wiki_cooc':
        path = root + 'wiki_cooc.mat'
    elif dataset == 'workers':
        path = root + 'workers.mat'
    elif dataset == 'minesweeper':
        path = root + 'minesweeper.mat'
    # data = sio.loadmat(path)
    data = scipy.io.loadmat(path)
    labels = torch.from_numpy(data['node_labels']).squeeze().long()
    adjs_labels = torch.from_numpy(data['adjoit_edge_ndarry']).float()
    adjs_labels = adjs_labels + torch.eye(adjs_labels.shape[0])  # self loop
    adjs = normalize_adj(adjs_labels)
    # adjs_labels = torch.from_numpy(data['label']).float()
    features = torch.from_numpy(data['node_features']).float()
    feature_labels = features
    shared_feature = features
    shared_feature_label = feature_labels
    num_graph = 1
    return labels, [adjs], [adjs_labels], shared_feature, shared_feature_label, num_graph
...
def load_data(dataset, path):
    ...
    if dataset in ['cora', 'citeseer', 'pubmed']:
    ...
    elif dataset in ['wiki_cooc', 'workers', 'minesweeper']:
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_new_data(dataset, path)
        ...
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_synthetic_data(r=0.00, path=path)
    elif dataset == 'acm01':
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_synthetic_data(r=0.10, path=path)
    elif dataset == 'acm02':
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_synthetic_data(r=0.20, path=path)
    elif dataset == 'acm03':
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_synthetic_data(r=0.30, path=path)
    elif dataset == 'acm04':
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_synthetic_data(r=0.40, path=path)
    elif dataset == 'acm05':
        labels, adjs, adjs_labels, shared_feature, shared_feature_label, num_graph = load_synthetic_data(r=0.50, path=path)
...
def cal_homo_ratio(adj, label, self_loop=True):
    ...
    y_onehot = np.eye(class_num)[label]  # 将多标签转换为单标签
    ...
    return homo_ratio, homo




# def cal_homo_ratio_new(adj, x_homos, self_loop=True):
#     adj_y = np.matmul(x_homos, x_homos.T)
#
#     if self_loop:
#         adj = adj - np.eye(adj.shape[0])
#
...
#     return homo_ratio, homo
def cal_homo_ratio_new(adj, x_homos, self_loop=True):
    # 将 NumPy 数组转换为 PyTorch 张量，并将其移到 GPU 上（如果可用）
    adj = torch.tensor(adj, dtype=torch.float64, device='cuda' if torch.cuda.is_available() else 'cpu')
    x_homos = torch.tensor(x_homos, dtype=torch.float64, device='cuda' if torch.cuda.is_available() else 'cpu')

    adj_y = torch.matmul(x_homos, x_homos.T)

    if self_loop:
        adj = adj - torch.eye(adj.size(0), device=adj.device)

    homo = torch.sum(adj_y * adj)
    homo_ratio = homo / torch.sum(adj)

    # 将结果移回到 CPU（如果在 GPU 上计算的话）
    homo_ratio = homo_ratio.item() if torch.cuda.is_available() else homo_ratio
    homo = homo.item() if torch.cuda.is_available() else homo
...
def load_synthetic_data(r=0.00, path='./data/'):
    ...
    for v in range(graph_num):
        adjs.append(normalize_adj(adjs_labels[v]))
    return labels, adjs, adjs_labels, shared_feature, shared_feature_label, graph_num
...
def cal_sep_loss(z_ho, z_he):
    S_z = torch.matmul(z_ho, z_he.T)
    loss = correlation_reduction_loss(S_z)
    return loss
...
def minmaxnormalize(x):
    ...
    x_normalized[torch.isinf(x_normalized)] = 0.
    return x_normalized
...
def remove_self_loop(A):
    A_new = torch.triu(A, diagonal=1) + torch.tril(A, diagonal=-1)
    return A_new
